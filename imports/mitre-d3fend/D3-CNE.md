---
slug: defense/D3-CNE
title: "D3-CNE — Content Excision"
page_type: defense
tags: [d3fend, defense, countermeasure, mitre]
d3fend_ids: [D3-CNE]
related: []
provenance: imported
import_source: mitre-d3fend
import_id: D3-CNE
updated_at: 2026-08-31
summary: "Removing specific, potentially malicious, parts of content"
sources:
  - name: MITRE D3FEND
    url: https://d3fend.mitre.org/technique/D3-CNE/
---

# D3-CNE: Content Excision

**MITRE D3FEND countermeasure**

## What it does

Removing specific, potentially malicious, parts of content

## Attacks this counters

No ATT&CK technique in this corpus maps to this countermeasure. D3FEND may map it to techniques outside the Enterprise matrix, or to ones MITRE has since revoked — absence here is about this corpus, not about the countermeasure.

## Source

- [MITRE D3FEND D3-CNE](https://d3fend.mitre.org/technique/D3-CNE/)
